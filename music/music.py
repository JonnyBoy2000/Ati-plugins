...
class Music:
    ...
    async def track_hook(self, player, event):
        ...
        if event == 'TrackStartEvent' and status:
            if playing_servers > 1:
                await self.bot.change_presence(activity=discord.Game(name='music in {} servers'.format(playing_servers)))
            else:
                await self.bot.change_presence(activity=discord.Game(name=get_single_title, type=2))
    ...
    async def queue(self, ctx, page: int=1):
        ...
        if player.current.stream:
            queue_list += '**Currently livestreaming:** [**{}**]({})\nRequested by: **{}**\n\n{}`{}`/`{}`\n\n'.format(player.current.title, player.current.uri, self.bot.get_user(player.current.requester), arrow, pos, dur)
        else:
            queue_list += 'Playing: [**{}**]({})\nRequested by: **{}**\n\n{}`{}`/`{}`\n\n'.format(player.current.title, player.current.uri, self.bot.get_user(player.current.requester), arrow, pos, dur)
    ...
    async def volume(self, ctx, vol: int=None):
        ...
        if not vol:
            ...
            embed = discord.Embed(colour=ctx.guild.me.top_role.colour, title='Current Volume:', description=str(vol) + '%')
            if not player.is_playing:
